...
> 💡 关于本工具
>
> 按需加载单个 Spine 资源包，主要用于方便个人参考或研究，不适合批量下载。
...
- **单人、单次使用**：本工具按需加载单个 Spine 资源包，主要用于方便个人参考或研究，不适合批量操作。
- **先解析，再决定**：`inspect` 只解析 `meta.json` 并输出资源 URL，不会自动下载纹理或骨骼文件。下不下载、下载哪些，由你自己决定。
- **按需刷新本地数据库**：`sync-db` 会把角色映射写到本地 `database/prts-characters.json`，日常 `generate` / `find` 只会读这个本地文件——需要更新时再跑一次就行。
- **素材来源**：本项目引用的明日方舟素材由 PRTS Wiki 托管，相关版权归鹰角网络所有。
